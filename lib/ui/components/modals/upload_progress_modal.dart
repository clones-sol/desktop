    ...
    for (final entry in queueItems.entries) {
      ...
      if (task.uploadStatus == UploadStatus.uploading &&
        ...
        if (elapsedMilliseconds > 0) {
          _uploadSpeeds[recordingId] =
              (task.uploadedBytes / elapsedMilliseconds) * 1000;
        }
      }
                  ...
                  if (queueItems.isEmpty)
                    ...
                    Expanded(
                      child: SingleChildScrollView(
                        child: Column(
                          children: queueItems.map(_buildUploadItem).toList(),
                        ),
                      ),
                    ),
...
Widget _buildFooterActions(
  List<UploadTaskState> queueItems,
  void Function(String) removeUpload,
) {
  return Column(
      ...
      Row(
        ...
        children: [
          ...
          if (queueItems.any(
              ...
              queueItems.any(
                (item) => item.uploadStatus == UploadStatus.error,
              ))
...
Color getStatusColor(UploadStatus status, BuildContext context) {
  switch (status) {
    case UploadStatus.done:
      return ClonesColors.containerIcon4;
    case UploadStatus.error:
      return ClonesColors.containerIcon3;
    case UploadStatus.processing:
    case UploadStatus.uploading:
    case UploadStatus.zipping:
      return ClonesColors.containerIcon5;
    case UploadStatus.idle:
      return ClonesColors.containerIcon1;
  }
}
